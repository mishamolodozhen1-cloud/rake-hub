-- Rake Hub-Beta [ENHANCED ESP + FIXED RAKE]
...
local WIDTH = isMobile and 240 or 340
local HEIGHT = isMobile and 320 or 440
...
local function CreateEspElements(object, color, text, useHighlight, espType)
    ...
    if useHighlight then
        ...
        Highlight.FillTransparency = 0.55; -- Сделал заливку более плотной
    ...
    Billboard.Size = UDim2.new(0, 250, 0, 45) -- Сильно увеличил размер рамки текста
    Billboard.StudsOffset = Vector3.new(0, 3, 0) -- Поднял чуть выше, чтобы текст не сливался с моделькой
    ...
    Label.TextStrokeTransparency = 0 -- Добавил максимальную черную обводку для контраста
    Label.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
    Label.TextSize = isMobile and 16 or 20 -- Сделал шрифт ОГРОМНЫМ
...
-- УЛУЧШЕННЫЙ ПОИСК РЕЙКА (Сканирует ВЕСЬ Workspace, а не только Children)
...
local function GetRake()
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and not Players:GetPlayerFromCharacter(obj) then
            ...
            if string.find(n, "rake") or string.find(n, "monster") then
...
CreateElement("ESP Монстр", function(state) Settings.EspMonster = state end, false, Color3.fromRGB(255, 30, 30))
